...
selected_vars <- c("src_subject_id", "eventname", "cbcl_scr_syn_external_t")
...
df_wide <- pivot_wider(
    ...
    values_from = cbcl_scr_syn_external_t,
    id_cols = src_subject_id
)

df_wide <- df_wide %>%
    rename(
        extern.0 = baseline_year_1_arm_1,
        extern.1 = `1_year_follow_up_y_arm_1`,
        extern.2 = `2_year_follow_up_y_arm_1`,
        extern.3 = `3_year_follow_up_y_arm_1`,
        extern.4 = `4_year_follow_up_y_arm_1`
    )

## Subset the nonimaging data to include desired variables
selected_vars_wide <- c("extern.0", "extern.1", "extern.2", "extern.3", "extern.4")
...
descriptives_table <- df_long %>%
  select(eventname, cbcl_scr_syn_external_t) %>%
  mutate(eventname = factor(eventname, levels = c("baseline_year_1_arm_1", "1_year_follow_up_y_arm_1", "2_year_follow_up_y_arm_1", "3_year_follow_up_y_arm_1", "4_year_follow_up_y_arm_1"), labels = c("Baseline", "Year 1", "Year 2", "Year 3", "Year 4"))) %>%
  tbl_summary(
    by = eventname, 
    missing = "no",
    label = list(cbcl_scr_syn_external_t ~ "Externalizing Score"),
    statistic = list(all_continuous() ~ "{mean} ({sd})")
  )

# Now, descriptives_table contains the summary, and you can print or view it
descriptives_table
...
knitr::kable(desc, caption = "Item descriptives", digits = 2)
...
# df_long Plots
ggplot(df_long, aes(x = cbcl_scr_syn_external_t)) +
    geom_density() +
...
df_scores <- df_long
# Store original range of cbcl_scr_syn_external_t
rng_extern <- range(df_scores$cbcl_scr_syn_external_t, na.rm = TRUE)
# Log-transform
df_scores$log <- scales::rescale(log(df_scores$cbcl_scr_syn_external_t), to = c(
    0,
    1
))
# Square root transform
df_scores$sqrt <- scales::rescale(sqrt(df_scores$cbcl_scr_syn_external_t), to = c(
    0,
    1
))
# Cube root transform
df_scores$qrt <- scales::rescale(df_scores$cbcl_scr_syn_external_t^0.33, to = c(
    0,
    1
))
# Reciprocal transform
df_scores$reciprocal <- scales::rescale(1 / df_scores$cbcl_scr_syn_external_t, to = c(
    0,
    1
))
...
# Box-Cox transform
b <- MASS::boxcox(lm(df_scores$cbcl_scr_syn_external_t ~ 1), plotit = FALSE)
lambda <- b$x[which.max(b$y)]
df_scores$boxcox <- bc(df_scores$cbcl_scr_syn_external_t, lambda)
...
# Rescale cbcl_scr_syn_external_t
df_scores$cbcl_scr_syn_external_t <- scales::rescale(df_scores$cbcl_scr_syn_external_t, to = c(0, 1))
...
df_plot <- do.call(rbind, lapply(c(
    "cbcl_scr_syn_external_t", "log", "sqrt", "qrt",
    "boxcox"
), function(n) {
...
ggplot(df_plot, aes(x = Value, colour = Transformation)) +
    ...
    facet_wrap(~eventname) +
    scale_y_sqrt() +
    xlab("Externalizing (rescaled to 0-1)") +
    theme_bw()
```
...
names(dat) <- gsub("boxcox.", "extern", names(dat))
```

#### Model Selection

A table of model characteristics and fit statistics is presented in Table X. For all models a reduction in BIC, saBIC and AIC was observed with increasing class number. The reduction in ICs flattened after three classes. In addition, the LoMendell-Rubin (LMR) likelihood ratio test was significant with every additional class and entropy was above.90 for all models. Models with xx or more classes were not accepted. For all models with one to three classes, the class size was larger than 5% and consisted of at least 50 individuals.
...
res_step <- mx_growth_mixture(
    model = "
  i =~ 1*externbaseline_year_1_arm_1 + 1*extern1_year_follow_up_y_arm_1 + 1*extern2_year_follow_up_y_arm_1 +1*extern3_year_follow_up_y_arm_1 +1*extern4_year_follow_up_y_arm_1
  step =~ 0*externbaseline_year_1_arm_1 + 1*extern1_year_follow_up_y_arm_1 + 1*extern2_year_follow_up_y_arm_1 +1*extern3_year_follow_up_y_arm_1 +1*extern4_year_follow_up_y_arm_1
  s =~ 0*externbaseline_year_1_arm_1 + 0*extern1_year_follow_up_y_arm_1 + 1*extern2_year_follow_up_y_arm_1 +2*extern3_year_follow_up_y_arm_1 +3*extern4_year_follow_up_y_arm_1
  externbaseline_year_1_arm_1 ~~ vexternbaseline_year_1_arm_1*externbaseline_year_1_arm_1
  extern1_year_follow_up_y_arm_1 ~~ vextern1_year_follow_up_y_arm_1*extern1_year_follow_up_y_arm_1
  extern2_year_follow_up_y_arm_1 ~~ vextern2_year_follow_up_y_arm_1*extern2_year_follow_up_y_arm_1
  extern3_year_follow_up_y_arm_1 ~~ vextern3_year_follow_up_y_arm_1*extern3_year_follow_up_y_arm_1
  extern4_year_follow_up_y_arm_1 ~~ vextern4_year_follow_up_y_arm_1*extern4_year_follow_up_y_arm_1
  i ~~ 0*i
  step ~~ 0*step
  s ~~ 0*s
  i ~~ 0*s
...
The xx model also evidenced convergence problems, but this (as we will see) is because the solution is overfitted.
